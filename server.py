...
# Reusable business/data logic for the labeling features lives in these modules;
# server.py stays the HTTP/request-routing shell that calls into them.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _api_detail(self, conv_id):
        ...
        try:
            ...
            conv_dict["label"] = labels.conv_label(conn, conv_id)
    ...
    def _api_snapshot_restore(self, sid):
        payload = self._read_json_body()
        self._handle_db(lambda conn: snapshots.snapshot_restore(conn, sid, payload))
